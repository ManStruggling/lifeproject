require(["../scripts/config.js"], function () {
    require(["jquery", "cookie", "benlailife", "randomCode"], function ($, cookie) {
        let phone_success = false;
        let password_success = false;
        let pass_comfirm_success = false;
        let register_yzm = false;
        //手机号的正则验证
        ...
        //验证码
        $('.register_yzm span').createCode({
            len: 6
        })
        $('.register_yzm input').blur(function () {
            if ($(this).val().toLowerCase() != $('.register_yzm span').children('input').val().toLowerCase()) {
                $(".register_yzm p").html("验证码输入有误!");
                register_yzm = false;
            } else {
                $(".register_yzm p").html("");
                register_yzm = true;
        ...
        $(".register_password_txt").blur(function () {
            ...
            if ((has_num + has_small + has_large >= 2) && reg.test($(this).val())) {
                ...
                $(".register_password p").html("密码须为8-16位大写字母,小写字母,数字,两种组合以上!");
        ...
        function register() {
            if ( phone_success && password_success && pass_comfirm_success && $("#memory").is(':checked') && register_yzm ) {
